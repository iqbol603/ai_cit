...
function init() {
    if (typeof quizData !== 'undefined') {
        ...
        document.getElementById('quiz-content').innerHTML = `<p style="color:red">❌ Хато: data.js ёфт нашуд!</p>`;
    }
}
...
function renderMenu() {
    ...
    html += `<button class="section-btn all" onclick="setupQuiz('all')">
                <span>🌍 Ҳамаи бахшҳо</span><small>${totalAll} савол</small>
             </button>`;

    allSections.forEach((item, index) => {
        html += `<button class="section-btn" onclick="setupQuiz(${index})">
                    <span>📦 ${item.section}</span><small>${item.questions.length} савол</small>
                 </button>`;
    });
...
function displayQuestion() {
    ...
    quizContent.innerHTML = `
        ...
        <button id="skip-btn" class="next-btn skip-btn" onclick="skip()">Гузаштан ⏭️</button>
...
function handleTimeOut() {
    check({classList: {add: ()=>{}}}, null, currentQuestions[currentQuestionIndex].correct_answer, "Вақт тамом шуд!");
}

function skip() { if (confirm("Гузаштан?")) { if (timerInterval) clearInterval(timerInterval); goToNext(); } }
